...
export class AdminService implements OnModuleInit {
  ...
  constructor(
    ...
    private complaintModel: Model<AdminComplaint>,
    @InjectModel(AdminRecipient.name)
    private recipientModel: Model<AdminRecipient>,
    @InjectModel(AdminInventory.name)
  ...
  private async refreshCaches() {
    const [requests, complaints, recipients, inventory, campaigns] =
      await Promise.all([
    ...
    this.recipientsCache = recipients.map((recipient: any) => ({
      ...
      requestDate:
        recipient.requestDate ?? new Date(recipient.createdAt).toISOString(),
    }));
  ...
  private async buildDonationChart() {
    ...
    const donorDocs = await this.donorModel
  ...
  private async buildDemandChart() {
    const requestDocs = await this.requestModel
      .find()
    ...
    requestDocs.forEach((doc: any) => {
      counts.set(
        doc.bloodType,
        (counts.get(doc.bloodType) ?? 0) + Number(doc.quantity ?? 0),
      );
    });

    const bloodTypes =
      this.inventoryCache.length > 0
        ? this.inventoryCache.map((item) => item.bloodType)
        : ['O+', 'O-', 'A+', 'A-', 'B+', 'B-', 'AB+', 'AB-'];
  ...
  private buildReportSummaries() {
    ...
    const pendingRequests = this.requestsCache.filter(
      (request) => request.status === 'Pending',
    ).length;
    const totalUnits = this.inventoryCache.reduce(
      (sum, item) => sum + item.available,
      0,
    );
    const openComplaints = this.complaintsCache.filter(
      (complaint) => complaint.status === 'Pending',
    ).length;
    const hasData =
      totalRequests > 0 ||
      totalUnits > 0 ||
      this.complaintsCache.length > 0 ||
  ...
  async getDashboardStats() {
    ...
    const availableUnits = this.inventoryCache.reduce(
      (sum, item) => sum + item.available,
      0,
    );
  ...
  async getComplaints(page = 1, limit = 10) {
    ...
    const {
      ...
      page: currentPage,
      limit: currentLimit,
    } = this.paginate(this.complaintsCache, page, limit);
  ...
  async getReports(page = 1, limit = 10) {
    ...
    const {
      ...
      page: currentPage,
      limit: currentLimit,
    } = this.paginate(reports, page, limit);
  ...
  async updateDonor(
    ...
    if (payload.status !== undefined)
      donor.availability = payload.status === 'Active';
  ...
  async getRequests(page = 1, limit = 10) {
    ...
    const {
      ...
      page: currentPage,
      limit: currentLimit,
    } = this.paginate(this.requestsCache, page, limit);
  ...
  async getRecipients(page = 1, limit = 10) {
    ...
    const {
      ...
      page: currentPage,
      limit: currentLimit,
    } = this.paginate(this.recipientsCache, page, limit);
  ...
  async updateComplaintStatus(
    id: string,
    payload: { status: ComplaintStatus; resolution?: string },
  ) {
  ...
  async exportReports(format: 'json' | 'csv' = 'json') {
    ...
    if (format === 'csv') {
      ...
      const rows = reports.map(
        (report) =>
          `${report.id},"${report.title.replace(/"/g, '""')}",${report.type},${report.status},${report.generatedDate}`,
      );
